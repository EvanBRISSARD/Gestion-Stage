...
require_once CHEMIN_RACINE . 'src/fonction.php';
...
switch ($page) {
    ...
    case 'etudiands.ad.ens': // ====== Section Pages AD/AP Étudiants
        if($enseignant['AP_ens'] || $enseignant['AD_ens']) {
    ...
    case 'planificationStage.ad.ens':
        if($enseignant['AP_ens'] || $enseignant['AD_ens']) {
            if ($sousPages == 'ajoutStage.ad.ens'){
                if($_SERVER['REQUEST_METHOD'] === 'POST'){
                    if(empty($_POST['date_debut']) && empty($_POST['date_fin']) && empty($_POST['annee_scolaire_etud'])){
                        header("Location: " . URL_RACINE . "enseignants.php?pages=planificationStage.ad.ens&sousPages=ajoutStage.ad.ens&error=1&message=Veuillez remplir le nom de la classe.");
                    } else {
                        require_once CHEMIN_RACINE . 'src/trait/ens/ad/AjoutStage.trait.php';
                    }
                    
                }

                require_once CHEMIN_RACINE . 'templates/ens/ad/AjoutStage.php';
                break;
            }

            if(isset($_GET['id_sta'])){
                require_once CHEMIN_RACINE . 'src/trait/ens/ad/SupStage.trait.php';
            }
